...
一个 npm 包，用户安装后在项目中生成持久化的记忆系统。通过编程助手（Claude Code / Cursor / CodeBuddy / Codex 等）提供的 hooks 能力，自动捕获使用上下文，逐步构建项目级记忆，让 AI 助手越来越了解这个仓库。
...
4. **渐进式采用** — `init` 多选助手；先实现 Claude Code adapter，再按注册表扩展 Cursor、CodeBuddy 等
...
> **当前实现基线**：npm `0.13.x`；`storage.backend: file`；已接入助手 `claude-code` / `cursor` / `codebuddy`（`init` 适配器均 `available: true`）。
...
**与 Phase 5 / Phase 11 的分工**：

| 机制 | 触发 | 目标层 |
|------|------|--------|
| `flush` → `promoteSkills` | 自动 consolidate | `.memory/skills/`（流程 → Skill） |
| `promote` CLI | 人工评审 | `.memory/topics/`（个人捕获 → 团队约定） |
| 同一 `.md.promote` 侧车 | Skill 晋升（flush）与团队 PR 候选（promote）可并存；procedural 在 promote PR 中提示优先走 Skill |
...
| --- | ------------------------------- | --------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0  | Claude Code                     | `claude-code`   | v0.1+      | Stop / SessionStart → `.claude/settings.local.json`（`hooks` 字段）                                                                                   |
| P1  | Cursor                          | `cursor`        | **v0.2.6** | `sessionStart` → inject；`stop` → capture；项目级 `[.cursor/hooks.json](https://cursor.com/docs/hooks)`（`version: 1` 扁平格式，与 Claude settings 不同）        |
| P1  | CodeBuddy（含 CodeBuddy Code CLI） | `codebuddy`     | **v0.7.0** | `SessionStart` / `Stop` → `.codebuddy/settings.local.json`（与 Claude Code Hooks 规范兼容，见 [CodeBuddy Hooks](https://www.codebuddy.ai/docs/cli/hooks)） |
...
- **后续版本**：新增适配器文件并注册即可；未实现的 id **不出现在可选列表**，或 UI 灰显且 CLI 拒绝 `--tools` 传入。
...
src/init/assistants/
  ...
  claude-code.ts # 合并写 .claude/settings.local.json
  cursor.ts      # v0.2.6，available: true
  codebuddy.ts   # v0.7 规划，available: false 直至实现
```
...
| ----------------------------------- | --------------------------------------------------------- | ----------------------------------------------------------------- |
| `claude-code`                       | `.claude/settings.local.json` → `capture` / `inject`      | v0.1                                                              |
| `cursor`                            | `.cursor/hooks.json`（`sessionStart` / `stop`）             | **v0.2.6**（`[cursor.ts](../src/init/assistants/cursor.ts)`）       |
| `codebuddy`                         | `.codebuddy/settings.local.json`（`SessionStart` / `Stop`） | **v0.7.0**（`[codebuddy.ts](../src/init/assistants/codebuddy.ts)`） |
...
- Phase 2 `capture`（**已实现**）：读取 `config.assistants`，仅当包含 `claude-code` 时处理 Claude Code 会话 JSONL。
- Phase 2c `capture` / `inject`（**v0.2.6 Cursor**）：当 `assistants` 含 `cursor` 时走 `src/capture/cursor/` 会话解析；hooks 仍统一为 `npx @riconext/hermes-repo capture` / `inject`（内部路由）。详见 [phase-2c-v0.2.6-cursor.md](phase-2c-v0.2.6-cursor.md)。
- Phase 2d `capture` / `inject`（**v0.7 CodeBuddy**）：当 `assistants` 含 `codebuddy` 时走 `src/capture/codebuddy/`（或复用 Claude 式 `transcript_path` 解析）；`init` 合并 `.codebuddy/settings.local.json`。详见下文「CodeBuddy」专章。
- 新助手上线：新增 adapter + 对应 `capture` 子路径即可。

```mermaid
flowchart LR
  init[init 多选 assistants] --> cfg[config.json assistants]
  cfg --> hooks[各助手 settings hooks]
...
| 消费者  | Claude Code / CodeBuddy hooks、AGENTS.md、CLI | Cursor / Claude / Codex 等一切支持 MCP 的助手     |
...
被动通道是核心：**Claude Code** 的 `Stop` hook 在每轮回复结束后触发，读取会话 JSONL 并落盘（v0.2）；**Cursor** 通过 `stop` 触发同一套 `capture` 流水线（v0.2.6）；**CodeBuddy**（含 CodeBuddy Code CLI）计划通过 `Stop` 触发（v0.7），与 Claude 类似由 stdin 提供 `transcript_path`。各助手细节见下文 Adapter 专章。

---

## 编程助手 Adapter 实现

各助手的 hooks 由 `init` 在用户勾选后写入（**仅 `available: true` 的适配器**）。**Claude Code**（`claude-code`）与 **Cursor**（`cursor`）已在 v0.2 / v0.2.6 实装；**CodeBuddy**（`codebuddy`）在 v0.7 按本章规格接入。

### Claude Code：hooks 配置（`.claude/settings.local.json`）

Claude Code 从 **settings** 文件加载 hooks，不读取独立的 `.claude/hooks.json`。`init` 每次合并写入 `settings.local.json` 的 `hooks.Stop` / `hooks.SessionStart`（个人层，通常不提交 git）。
...
### Cursor（P1，v0.2.6 已实现）

**当前代码状态**：`[src/init/assistants/cursor.ts](../src/init/assistants/cursor.ts)` `available: true`；`init` 写入 `.cursor/hooks.json`；捕获见 `[src/capture/cursor/](../src/capture/cursor/)`。实施记录：[phase-2c-v0.2.6-cursor.md](phase-2c-v0.2.6-cursor.md)。

**官方文档**：[Cursor Hooks](https://cursor.com/docs/hooks)（项目级配置路径为仓库根 `.cursor/hooks.json`，亦可用户级 `~/.cursor/hooks.json`）。

#### 与 Claude Code 的差异


| 维度            | Claude Code（v0.2 已实现）                      | Cursor（v0.2.6 已实现）                                                                                                                      |
| ------------- | ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| 配置文件          | `.claude/settings.local.json` 内 `hooks` 字段 | `.cursor/hooks.json`（顶层 `version: 1`）                                                                                                   |
| Hook 条目格式     | 嵌套数组 + `"type": "command"`                 | 事件名 → `[{ "command": "..." }]`（扁平）                                                                                                      |
| 记忆注入          | `SessionStart` → `inject`（stdout）          | `sessionStart` → `inject`（见下「注入方式」）                                                                                                     |
| 记忆捕获          | `Stop`（每轮 assistant 结束）                    | `stop`（agent loop 结束）；可选 `sessionEnd` 作会话级补捕                                                                                            |
| 会话 transcript | Stop stdin 提供 `transcript_path`            | **无**官方 `transcript_path`；`CursorSessionResolver` 根据 `session_id` + `~/.cursor/projects/<encoded>/agent-transcripts/` 解析（见 phase-2c 附录） |
| 失败语义          | hook 默认 exit 0（不阻塞）                        | 同上，与 `[hookExit](../src/hookExit.ts)` 一致                                                                                                |


#### 规划：`init` 写入的 `.cursor/hooks.json`

`init` 在用户勾选 `cursor` 后合并写入（保留用户已配置的其它事件，如 `beforeReadFile`、`afterFileEdit`）：
...
- 脚本路径：项目 hooks 以**仓库根**为 cwd，推荐 `.cursor/hooks/` 包装脚本或直接使用 `npx`（与 Claude 一致）。
- Windows：若使用无扩展名 shell 脚本，需按 [Cursor 文档](https://cursor.com/docs/hooks) 使用 `.cmd` 包装，避免系统用默认程序打开脚本。

#### 注入方式（`sessionStart`）

Cursor `sessionStart` 支持两类扩展上下文（实现时二选一或并存）：

1. **Command hook + stdout**（优先对齐 Claude）：若 Cursor 将 command 的 stdout 并入会话，则复用现有 `[runInject](../src/inject/runInject.ts)`（读 `.memory/MEMORY.md` 输出到 stdout）。
2. **JSON 响应 `additional_context`**（v0.2.6 已实现）：`sessionStart` hook stdin 含 `hook_event_name: sessionStart` 时，向 stdout 打印 `{ "additional_context": "<MEMORY 摘要>" }`。

`sessionStart` 为 fire-and-forget，不阻塞 Agent 启动；注入内容过长时仍遵守 Level 0 约 2200 字符上限。

#### 捕获方式（`stop` / 可选 `sessionEnd`）

```text
Cursor Agent 一轮结束（stop）
        │
        ▼
hermes-repo capture（config.assistants 含 cursor）
        │
        ▼
CursorSessionResolver：stdin 中的 session_id / workspace_roots
        → ~/.cursor/projects/<encoded>/agent-transcripts/<session-id>/<session-id>.jsonl
        │
        ▼
共用 shouldCapture → writeCapture → sessions/index.json
```

- `**stop**`：与 Claude `Stop` 最接近，在 agent loop 结束时触发；`status` 可为 `completed` / `aborted` / `error`（无价值时可 skip）。
- `**sessionEnd**`（可选）：composer 会话完全结束时再捕一次，避免仅依赖 `stop` 时漏掉长会话；若与 `stop` 重复落盘，由 `sessions/index.json` 去重或 capture 内按 `session_id` 幂等。
- **不用于记忆主路径的事件**：`workspaceOpen`（工作区级）、`beforeReadFile` / Tab hooks（与全局 MEMORY 注入无关）。

#### `capture` / `inject` 路由（v0.2.6）

```text
src/capture/
  hookInput.ts           # 统一 stdin 解析
  router.ts              # assistants 含 cursor → runCursorCapture()
  cursor/
    resolveSession.ts
    run.ts
```

- `config.assistants` **仅含 `cursor`** 时：不调用 Claude `~/.claude/projects/...` JSONL 逻辑。
- `config.assistants` **同时含 `claude-code` 与 `cursor`**：各自 hook 独立触发，各自路由；共享 `.memory/captures/` 与 `shouldCapture` 规则。
...
| 项                                    | 行为                                                   |
| ------------------------------------ | ---------------------------------------------------- |
| `init` 多选                            | Cursor 已在 checkbox（`label: "Cursor"`）                |
| `init -y --tools claude-code,cursor` | 写入两套 hooks + `assistants` 并集                         |
| `AGENTS.md`                          | 仍通用；可在「已启用助手」列出 Cursor — hooks: `.cursor/hooks.json` |


#### v0.2.6 验收标准

1. `init -y --tools cursor` 生成 `.cursor/hooks.json`，且 `config.assistants` 含 `cursor`。
2. Cursor Agent **新会话**能加载 `MEMORY.md` 摘要（inject 路径验证）。
3. Agent **一轮结束后**在可解析会话源的前提下写入 `.memory/captures/`。
4. 仅启用 Cursor、未启用 Claude 时，不依赖 Claude JSONL 路径。
5. `bun run test` 含 Cursor hooks 模板与 router 分支的 fixture 测试。

#### 明确不在 v0.2.6 范围

- Cursor **Tab** 补全 hooks（`beforeTabFileRead` / `afterTabFileEdit`）。
- 基于 `beforeReadFile` 的逐文件审计（非 hermes-repo 记忆模型）。
- MCP 直连记忆服务（v2.0 另仓）。

### CodeBuddy（P1，v0.7.0 已实现）

**产品范围**：本节 **一个** `assistants` id — `codebuddy` — 覆盖：

- **CodeBuddy Code CLI**（终端 Agent，`codebuddy` 命令；Hooks 主战场）；
- **CodeBuddy IDE / 插件**：若与 CLI 共用项目级 `.codebuddy/settings*.json`，与 CLI **同一套** hooks 与 `capture`/`inject`，不单独拆 `codebuddy-cli` id。

**官方文档**：

- [Hooks Reference](https://www.codebuddy.ai/docs/cli/hooks)（与 Claude Code Hooks 规范兼容，CodeBuddy Code **v1.16.0+**）
- [Settings Configuration](https://www.codebuddy.ai/docs/cli/settings)
- [Hooks Guide](https://www.codebuddy.ai/docs/cli/hooks-guide)

**当前代码状态**：`[src/init/assistants/codebuddy.ts](../src/init/assistants/codebuddy.ts)` `available: true`；捕获见 `[src/capture/codebuddy/](../src/capture/codebuddy/)`。实施记录：[phase-2d-v0.7.0-codebuddy.md](phase-2d-v0.7.0-codebuddy.md)。

#### 与 Claude Code / Cursor 的差异


| 维度            | Claude Code（v0.2）                       | Cursor（v0.2.6）                                      | CodeBuddy（v0.7.0）                                                                                                                         |
| ------------- | --------------------------------------- | --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| 配置文件          | `.claude/settings.local.json`           | `.cursor/hooks.json`                                | `.codebuddy/settings.local.json`（亦可 `.codebuddy/settings.json` 项目共享层）                                                                     |
| Hook 格式       | 嵌套 `type: "command"`                    | 扁平 `command`                                        | **同 Claude**：嵌套 `matcher` + `hooks[]`                                                                                                     |
| 记忆注入          | `SessionStart` → `inject`（stdout 并入上下文） | `sessionStart` → JSON `additional_context` 或 stdout | `SessionStart` → `inject`（**stdout**，与 Claude 一致）                                                                                         |
| 记忆捕获          | `Stop`                                  | `stop`                                              | `Stop`（主 Agent 每轮结束；`SubagentStop` 可选后续）                                                                                                  |
| 会话 transcript | stdin `**transcript_path`**             | 无官方字段；`session_id` + `~/.cursor/projects/...`       | stdin `**transcript_path**`（与 Claude 同族）                                                                                                  |
| 配置作用域         | 用户 / 项目 / local                         | 项目 / 用户                                             | 用户 `~/.codebuddy/settings.json`、项目 `.codebuddy/settings.json`、**local** `.codebuddy/settings.local.json`（hermes **init 写 local**，不提交 git） |
| 环境变量          | `CLAUDE_CONFIG_DIR`                     | —                                                   | `CODEBUDDY_CONFIG_DIR`（规划，默认 `~/.codebuddy`）；hook 运行时 `CODEBUDDY_PROJECT_DIR`                                                             |


#### 规划：`init` 写入的 `.codebuddy/settings.local.json`

`init` 在用户勾选 `codebuddy` 后合并写入（**仅覆盖** hermes 管理的 `Stop`、`SessionStart`，保留用户其它事件如 `PreToolUse`、`PostToolUse`、`Notification` 等）：
...
- 与 Claude 模板 `[hooks.json.tpl](../templates/hooks.json.tpl)` 结构一致，可复用 `mergeClaudeLocalSettings` 的合并策略，实现为 `mergeCodebuddyLocalSettings`（路径常量 `.codebuddy/settings.local.json`）。
- 项目级脚本推荐 `"$CODEBUDDY_PROJECT_DIR"/.codebuddy/hooks/...`（见官方文档）；hermes 默认仍用 `npx` 以降低路径耦合。
- Windows：Hook 命令须在 **Git Bash** 下可执行（CodeBuddy 与 Claude 相同约束）；避免无扩展名脚本被错误关联。

#### 注入方式（`SessionStart`）

- **Command hook + stdout**（与 Claude 对齐）：`[runInject](../src/inject/runInject.ts)` 读 `.memory/MEMORY.md` 输出纯文本；CodeBuddy 将 stdout **追加到上下文**（非 Cursor 的 `additional_context` JSON）。
- `SessionStart` matcher 可选：`startup` / `resume` / `clear` / `compact` — v0.7 **不配置 matcher**，匹配所有 SessionStart（与当前 Claude init 行为一致）。
- Level 0 约 2200 字符上限不变。

#### 捕获方式（`Stop`）

```text
CodeBuddy Code CLI 主 Agent 一轮结束（Stop）
        │
        ▼
hermes-repo capture（config.assistants 含 codebuddy）
        │
        ▼
hook stdin：transcript_path（首选）+ session_id + cwd
        │
        ▼
CodeBuddySessionResolver：
  1. HERMES_CODEBUDDY_SESSION（测试）
  2. transcript_path（hook / stdin）
  3. ~/.codebuddy/projects/<encoded-cwd>/*.jsonl（与 Claude projects 布局类似）
        │
        ▼
parseJsonlFile → shouldCapture → writeCapture → sessions/index.json（assistant: "codebuddy"）
```

**Stop hook stdin 示例**（官方 schema）：

```json
{
  "session_id": "abc123",
  "transcript_path": "/Users/xxx/.codebuddy/projects/.../00893aaf-....jsonl",
  "cwd": "/path/to/repo",
...
**路由判别**（与 Claude 并存时）：`hookInput.transcript_path` 路径含 `/.codebuddy/` → `runCodebuddyCapture`；含 `/.claude/` → `runClaudeCodeCapture`；仅启用一方助手时按 `assistants` 过滤。`hook_event_name` 为 `Stop` / `SessionStart`（PascalCase），**不是** Cursor 的 `stop` / `sessionStart`。

**明确不在 v0.7 主路径**：

- `SubagentStop` / `SessionEnd` 补捕（可选 follow-up）；
- `type: "prompt"` 的 LLM 判定型 hooks；
- `PreToolUse` 审计链（非记忆模型）。

#### `capture` / `inject` 路由（v0.7）

```text
src/capture/
  hookInput.ts              # 已有；扩展路径判别 .codebuddy vs .claude
  router.ts                 # assistants 含 codebuddy → runCodebuddyCapture()
  codebuddy/                # 新增（或薄封装 claude-code/resolver + 独立 config 根）
    resolveSession.ts       # CODEBUDDY_CONFIG_DIR、encodeCodebuddyProjectDir
    run.ts
src/init/
  mergeCodebuddySettings.ts
  assistants/codebuddy.ts
templates/
  hooks.codebuddy.json.tpl  # 可与 hooks.json.tpl 同构
```

- `config.assistants` **仅含 `codebuddy`** 时：不读 `~/.claude/projects`、不读 Cursor `~/.cursor/projects`。
- **同时含 `claude-code` 与 `codebuddy`**：各自 hook 独立触发；按 `transcript_path` 前缀或 `assistants` + 无 stdin 时的顺序回退（先 Claude 再 CodeBuddy，与双助手 Cursor 策略类似）。
...
| 项                                              | 行为                                                           |
| ---------------------------------------------- | ------------------------------------------------------------ |
| `init` 多选                                      | 增加 `label: "CodeBuddy（CLI）"` checkbox                        |
| `init -y --tools codebuddy`                    | 写入 `.codebuddy/settings.local.json` + `assistants` 并集        |
| `init -y --tools claude-code,codebuddy,cursor` | 三套 hooks 配置 + 单一 `config.assistants`                         |
| CLI 面板                                         | 用户可在 CodeBuddy 内 `/hooks` 审阅配置变更（产品能力，hermes 不实现 UI）         |
| `AGENTS.md`                                    | 通用；可列出「已启用助手：… codebuddy → `.codebuddy/settings.local.json`」 |


#### v0.7 验收标准

1. `init -y --tools codebuddy` 生成 `.codebuddy/settings.local.json`，且 `config.assistants` 含 `codebuddy`。
2. CodeBuddy Code CLI **新会话**能注入 `MEMORY.md` 摘要（`SessionStart` + stdout）。
3. **Stop** 触发 `capture` 且 router 走 codebuddy 分支（`debug` 可见 `transcript_path`）。
4. 仅启用 CodeBuddy 时不依赖 Claude / Cursor JSONL 路径。
5. `bun run test` 含 merge settings、router、`HERMES_CODEBUDDY_SESSION` fixture。
6. Hook 失败仍 exit 0；单 hook 超时 60s 由 CodeBuddy 运行时处理，hermes 保持轻量快速返回。

#### 实施备注（降低重复建设）

- **init**：`mergeCodebuddyLocalSettings` 与 `[mergeClaudeLocalSettings](../src/init/mergeClaudeSettings.ts)` 可抽共享 `mergeNestedCommandHooks(settingsPath, template)`。
- **capture**：JSONL 解析优先 **复用** `[parseJsonlFile](../src/capture/claude-code/parseJsonl.ts)`；项目目录编码规则需在实机确认是否与 Claude「路径 `/` → `-`」完全一致，否则单独 `encodeCodebuddyProjectDir`。
- **类型**：`AssistantId` 增加 `"codebuddy"`；`[registry.ts](../src/init/assistants/registry.ts)` 注册适配器。
...
| 版本 | 内容 | 预计规模 |
|------|------|---------|
| v0.1 | init：目录 + AGENTS.md + 助手多选（仅 `claude-code`）+ `config.assistants` + `.claude/settings.local.json` | ~150 行 TS |
...
| v0.8 | 跨仓库支持（全局索引 + 按 repo 标签）— **暂缓** | ~200 行 TS |
| v0.2.6 | **Cursor**：`cursor`、`init` 写入 `.cursor/hooks.json`、`capture/cursor`、router | 已完成 |
| v0.7 | **CodeBuddy**：`codebuddy` 可用、`mergeCodebuddyLocalSettings`、`capture/codebuddy`（`transcript_path` + `~/.codebuddy/projects`）、router 判别 | ~120–180 行 TS |
| v1.0 | curator：陈旧捕获清理 + 技能低活跃降级 + 归档 | ~250 行 TS |
| v1.1 | hermes-repo：`storage` 抽象 + `config.storage.backend` 预留；可选仓库内 SQLite FTS5 — **暂缓** | ~150 行 TS |
